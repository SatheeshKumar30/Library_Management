...
public  void login() {
    ...
    login_but.addActionListener(new ActionListener() {  //Perform action
        ...
        String username = F_user.getText(); //Store user name entered by the user in the variable "user name"
        @SuppressWarnings("deprecation")
		String password = F_pass.getText(); //Store password entered by the user in the variable "password"
         
        if(username.equals("")) //If user name is null
        {
        ...
        else if(password.equals("")) //If password is null
        {
            ...
            Connection connection=db.connect();  //Connect to the database
            try
            {
              ...
              if(rs.next()==false) { //Move pointer below
                ...
                while(rs.next())
                {
                  ...
                  if(admin.equals("1")) { //If boolean value 1
                	  ...
                	  am.admin_menu(); //redirect to admin menu
                  }
                  else{
                	  User_Menu um = new User_Menu();
                	  um.user_menu(UID); //redirect to user menu for that user ID
                  }
              }
              }
            }
    ...
    f.add(l1);  // add label1 i.e. for user name
